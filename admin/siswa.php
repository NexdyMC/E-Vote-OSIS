...
if (isset($_GET['v']) && $_GET['v'] === 'false') {
    $pesan_alert = "<div class='p-4 mb-6 text-sm font-medium text-red-800 border border-red-100 rounded-xl bg-red-50'>Gagal menambahkan siswa. Pastikan nama &amp; kelas terisi.</div>";
}
...
<?php } ?>
  ...
  <?php if ($tokenBaru): ?>
  <div data-aos="fade-up" class="flex flex-col gap-4 p-5 mb-6 border rounded-2xl bg-primary-50 border-primary-100 sm:flex-row sm:items-center">
    <div class="flex items-center justify-center w-11 h-11 rounded-xl bg-primary-700 shrink-0">
    ...
    <div class="flex items-center gap-2 shrink-0">
      <code id="tokenBaruText" class="px-3 py-2 text-sm font-bold tracking-widest bg-white border rounded-lg border-primary-200 font-display text-primary-700"><?= htmlspecialchars($tokenBaru) ?></code>
      <button type="button" onclick="salinToken()" title="Salin token" class="flex items-center justify-center bg-white border rounded-lg w-9 h-9 border-primary-200 hover:bg-primary-100 text-primary-700">
  ...
  <div class="grid grid-cols-1 gap-6 mb-6 sm:grid-cols-3">
    <div data-aos="fade-up" class="bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_8px_24px_rgb(15,23,42,0.05)]">
      <p class="text-2xl font-bold font-display text-navy-900"><?= number_format($statistik['total_siswa'], 0, ',', '.') ?></p>
      <p class="mt-1 text-sm text-slate-500">Total Siswa</p>
    </div>
    <div data-aos="fade-up" data-aos-delay="80" class="bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_8px_24px_rgb(15,23,42,0.05)]">
      <p class="text-2xl font-bold font-display text-emerald-600"><?= number_format($statistik['sudah_voting'], 0, ',', '.') ?></p>
      <p class="mt-1 text-sm text-slate-500">Sudah Memilih</p>
    </div>
    <div data-aos="fade-up" data-aos-delay="160" class="bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_8px_24px_rgb(15,23,42,0.05)]">
      <p class="text-2xl font-bold font-display text-slate-400"><?= number_format($statistik['belum_voting'], 0, ',', '.') ?></p>
      <p class="mt-1 text-sm text-slate-500">Belum Memilih</p>
    </div>
  </div>

  <!-- ============ HEADER + TOMBOL TAMBAH ============ -->
  <div data-aos="fade-up" class="flex items-center justify-between gap-4 mb-6">
    <div>
      <h2 class="font-semibold font-display text-navy-900">Daftar Siswa</h2>
      <p class="mt-1 text-sm text-slate-500"><?= count($siswa_list) ?> siswa terdaftar</p>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-slate-100 bg-slate-50/60">
            <th class="w-12 px-5 py-3 font-semibold text-left text-slate-500">No</th>
            <th class="px-5 py-3 font-semibold text-left text-slate-500">Token</th>
            <th class="px-5 py-3 font-semibold text-left text-slate-500">Nama</th>
            <th class="px-5 py-3 font-semibold text-left text-slate-500">Kelas</th>
            <th class="px-5 py-3 font-semibold text-left text-slate-500">Status Voting</th>
            <th class="px-5 py-3 font-semibold text-right text-slate-500">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <?php if (empty($siswa_list)): ?>
          <tr>
            <td colspan="6" class="px-5 py-10 text-center text-slate-400">Belum ada data siswa.</td>
          </tr>
          ...
          <tr class="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
            ...
            <td class="px-5 py-3.5">
              <code class="px-2 py-1 text-xs font-semibold tracking-wider rounded-md bg-slate-100 text-navy-700"><?= htmlspecialchars($s['token']) ?></code>
            </td>
            ...
            <td class="px-5 py-3.5 text-right">
              <form action="hapus-siswa.php" method="POST" class="inline"
                ...
                <button type="submit" title="Hapus" class="flex items-center justify-center ml-auto text-red-600 rounded-lg w-9 h-9 bg-red-50 hover:bg-red-100">
  ...
  <!-- ============ MODAL TAMBAH SISWA ============ -->
  <div id="modalWrapSiswa" class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-hidden">
    <div class="absolute inset-0 modal-overlay-bg bg-navy-950/60 backdrop-blur-sm" onclick="closeModalSiswa()"></div>

    <div class="relative w-full max-w-md p-6 bg-white shadow-2xl modal-panel rounded-3xl sm:p-8">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold font-display text-navy-900">Tambah Siswa Baru</h3>
        <button type="button" onclick="closeModalSiswa()" class="flex items-center justify-center rounded-full w-9 h-9 hover:bg-slate-100 text-slate-400">
      ...
      <form action="simpan-siswa.php" method="POST" class="space-y-5">
        ...
        <p class="-mt-2 text-xs text-slate-400">Token voting dibuat otomatis oleh sistem setelah data disimpan.</p>

        <div class="flex items-center gap-3 pt-2">
          <button type="button" onclick="closeModalSiswa()" class="flex-1 px-5 py-3 text-sm font-semibold transition border rounded-xl border-slate-200 text-navy-700 hover:bg-slate-50">Batal</button>
