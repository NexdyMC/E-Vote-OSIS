<?php
/* =========================================================
   PEMILU OSIS — SMK INFORMATIKA SUMEDANG
   ---------------------------------------------------------
   Contoh logika PHP sederhana untuk mengecek status login
   siswa via session. Di aplikasi nyata, $_SESSION['user']
   diisi setelah proses login berhasil (cek ke database/API).
   ========================================================= */
session_start();

// --- SIMULASI STATE LOGIN (hapus/ganti dengan proses login asli) ---
// Untuk mencoba tampilan "sudah login", uncomment baris di bawah:
// $_SESSION['user'] = ['nama' => 'Rangga Saputra', 'nis' => '2425.10.045'];

$isLoggedIn = isset($_SESSION['user']);
$namaSiswa  = $isLoggedIn ? $_SESSION['user']['nama'] : null;

// --- DATA KANDIDAT (idealnya diambil dari API/database) ---
$kandidat = [
    [
        'nomor' => 1,
...
<section id="beranda" class="relative overflow-hidden bg-[#F8FAFC] pt-40 pb-32">
  ...
  <div class="relative max-w-6xl mx-auto px-6 text-center">
    <span class="inline-block text-xs sm:text-sm font-semibold tracking-widest text-primary-700 bg-primary-50 px-4 py-1.5 rounded-full uppercase">
      Pemilihan Ketua &amp; Wakil Ketua OSIS 2026/2027
    </span>

    <h1 class="font-display font-extrabold text-4xl sm:text-5xl lg:text-6xl leading-tight mt-6 text-navy-900">
      Satu Suara,<br>
      <span class="text-primary-700">Satu Arah</span> untuk
      <span class="relative inline-block">
        OSIS
        <svg class="absolute -bottom-2 left-0 w-full" height="10" viewBox="0 0 200 10" preserveAspectRatio="none">
          <path d="M0,8 Q50,0 100,8 T200,8" stroke="#FACC15" stroke-width="6" fill="none" stroke-linecap="round"/>
        </svg>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      <?php foreach ($kandidat as $k): ?>
        <div class="card-kandidat relative bg-white rounded-3xl shadow-[0_8px_30px_rgb(15,23,42,0.06)] overflow-hidden border border-slate-100">
          ...
          <!-- Badge nomor urut -->
          <span class="absolute top-4 left-4 z-10 w-11 h-11 rounded-full bg-accent-400 text-navy-900 font-display font-bold flex items-center justify-center shadow-md shadow-accent-500/30">
            <?= $k['nomor'] ?>
          </span>

          <img src="<?= $k['foto'] ?>" alt="Foto Paslon <?= $k['nomor'] ?>" class="w-full h-56 object-cover">

          <div class="p-6">
            <h3 class="font-display font-semibold text-lg text-navy-900 mb-2"><?= htmlspecialchars($k['nama']) ?></h3>
            <p class="text-sm text-navy-600 leading-relaxed mb-6"><?= htmlspecialchars($k['visi']) ?></p>

            <?php if ($isLoggedIn): ?>
              <!-- ===== SUDAH LOGIN: tombol vote aktif ===== -->
              <form action="vote.php" method="POST">
                <input type="hidden" name="nomor_paslon" value="<?= $k['nomor'] ?>">
                <button type="submit"
                  class="btn-cta w-full font-display font-semibold text-sm px-5 py-3 rounded-xl bg-primary-700 text-white hover:bg-primary-600 flex items-center justify-center gap-2">
                  Pilih Paslon <?= $k['nomor'] ?>
                </button>
              </form>
            <?php else: ?>
              <!-- ===== BELUM LOGIN: tombol terkunci ===== -->
              <a href="login.php"
...
<!-- ============================================================
     SECTION 4 — COUNTDOWN VOTING (DARK)
     ============================================================ -->
<section id="countdown" class="relative bg-navy-900 pt-28 pb-32 overflow-hidden">
  ...
  <div class="relative max-w-4xl mx-auto px-6 text-center">
    ...
    <div class="relative mt-14 grid grid-cols-4 gap-3 sm:gap-6 max-w-2xl mx-auto">
      <!-- glowing horizontal connector, sits behind the 4 boxes -->
...
<section id="tips" class="relative bg-[#F8FAFC] pt-28 pb-32 overflow-hidden">
  ...
  <div class="relative max-w-7xl mx-auto px-6">
    ...
    <!-- Wrapper Horizontal Line & Cards -->
    <div class="relative">
      
      <!-- Horizontal Timeline Line (Hanya muncul di layar sedang/besar) -->
...
<section id="video" class="relative bg-navy-900 pt-28 pb-32 overflow-hidden">
  ...
  <div class="relative max-w-3xl mx-auto px-6">
    ...
    <!--
      Teknik "facade": thumbnail ditampilkan dulu (ringan, tanpa lag),
      iframe YouTube baru dimuat saat tombol play diklik.
      Ini mencegah halaman menjadi berat karena embed YouTube langsung.
    -->
    <div id="videoFacade"
...
<script>
  ...
  // ===================== VIDEO FACADE (anti-lag) =====================
  // function muatVideoTutorial() {
  //   const facade = document.getElementById('videoFacade');
  //   facade.outerHTML = `
  //     <div class="relative aspect-video rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
  //       <iframe
  //         class="absolute inset-0 w-full h-full"
  //         src="https://www.youtube.com/embed/1rv1PjprD18?list=RD1rv1PjprD18"
  //         title="Video tutorial voting"
